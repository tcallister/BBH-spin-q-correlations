...
nWalkers = 32
output = "/home/thomas.callister/RedshiftDistributions/BBH-spin-q-correlations/code/output/emcee_samples_plPeak_gaussianQ"
...
if len(old_chains)==0:
    ...
    initial_mu_qs = np.random.random(nWalkers)
    initial_sig_qs = np.random.random(nWalkers)*0.9+0.1
    ...
    initial_walkers = np.transpose([initial_lmbdas,initial_mMaxs,initial_m0s,initial_sigMs,initial_fs,initial_mu_qs,initial_sig_qs,initial_ks,initial_mu0s,initial_sigma0s,initial_alphas,initial_betas])
...
# Run
nSteps = 10000
sampler = mc.EnsembleSampler(nWalkers,dim,logp_powerLawPeak_gaussianQ,args=[sampleDict,injectionDict,priorDict],threads=12)
